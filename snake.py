...
class Snake:
    ...
    def grow_body(self):
        ...
        last_segment = self.snake_body[len(self.snake_body) - 1]
        last_segment.color("white")
        last_segment.penup()
        previous_segment = self.snake_body[len(self.snake_body) - 2]
    ...
    def turn_up(self):
        if self.snake_body[0].heading() != 90 * 3:
            self.snake_body[0].setheading(90 * 1)

    def turn_down(self):
        if self.snake_body[0].heading() != 90 * 1:
            self.snake_body[0].setheading(90 * 3)

    def turn_left(self):
        if self.snake_body[0].heading() != 0:
            self.snake_body[0].setheading(90 * 2)

    def turn_right(self):
        if self.snake_body[0].heading() != 90 * 2:
            self.snake_body[0].setheading(90 * 0)
